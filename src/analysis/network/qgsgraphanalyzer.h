...
class ANALYSIS_EXPORT QgsGraphAnalyzer
{
  public:
     ...
     * \param endVertexIndexes optional set of desired end vertex indices. If specified the algorithm will be terminated as soon as the shortest path to all these vertices are found, potentially saving a lot of unwanted computation.
     ...
     */
    static void SIP_PYALTERNATIVETYPE( SIP_PYLIST ) dijkstra( const QgsGraph *source, int startVertexIdx, int criterionNum, const QSet< int > &endVertexIndexes = QSet< int >(), QVector<int> *resultTree = nullptr, QVector<double> *resultCost = nullptr );
